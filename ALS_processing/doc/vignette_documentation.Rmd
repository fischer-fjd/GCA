...
- **Description**
  ...
  9. Get pulse density and scan angle (as .tif files via ‘lasgrid’ function);
  
  10. Create DTMs (via ‘las2dem’ and/or ‘blast2dem’ functions);
    
  11. Create DSMs (via ‘las2dem’ and/or ‘blast2dem’ functions);
  
  12. Create thinned versions of DSMs (via ‘las2dem’ and/or ‘blast2dem’ functions);
    
  13. Create normalized point clouds (via ‘lasheight’ function);
    
  14. Estimate laser penetration (via ‘lasgrid’ function)
    
  15. Create CHMs (via ‘las2dem’, ‘lassort’, ‘lasoptimize’ and/or ‘lasclip’ functions);
    
  16. Compute canopy summary statistics from point clouds;
...
- Digital Terrain Model (DTM)
  ...
  Several versions of the DTM are generated by the pipeline:
    ...
    - TIN
      ...
      DTM generated using the ground classification generated by the pipeline, and genrating TIN over these ground points.
      
```{r BCI_DTM, echo=FALSE, message=FALSE, warnings=FALSE, fig.align = 'center', out.width = "50%", fig.cap = "DTM for the BCI dataset"}
knitr::include_graphics("resources/BCI_DTM.png")
```
...
```{r BCI_CHM, echo=FALSE, message=FALSE, warnings=FALSE, fig.align = 'center', out.width = "50%", fig.cap = "CHM for the BCI dataset"}
knitr::include_graphics("resources/BCI_CHM.png")
```
...
```{r BCI_pulsedensity, echo=FALSE, message=FALSE, warnings=FALSE, fig.align = 'center', out.width = "50%", fig.cap = "Pulse density raster for the BCI dataset"}
knitr::include_graphics("resources/BCI_pulsedensity.png")
```
...
```{r BCI_scanangle, echo=FALSE, message=FALSE, warnings=FALSE, fig.align = 'center', out.width = "50%", fig.cap = "Scan angle raster for the BCI dataset"}
knitr::include_graphics("resources/BCI_scanangle.png")
```
