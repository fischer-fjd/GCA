...
make.info = function(path_output = "", name_file = "_INFO_.txt", params_general){
  ...
  if(path_output != ""){
    ...
    version_pipeline = gsub("v","",gsub(".R","", path_script_split[length(path_script_split)][[1]], fixed = T))
  ...
  status = tryCatch(
    {
      ...
      if(n_cores > n_cores_automatic){
        cat("WARNING! It seems that the number of provides cores (", n_cores, ") is greater than the number of physical cores (", n_cores_automatic ,"). This may break the pipeline. Consider reducing the number of provided cores or setting n_cores to 'automatic'\n")
        if(file.exists(logfile)){
          logfile_dt = data.table(path = as.character(NA), issue = paste0("WARNING! It seems that the number of provides cores (", n_cores, ") is greater than the number of physical cores (", n_cores_automatic ,"). This may break the pipeline. Consider reducing the number of provided cores or setting n_cores to 'automatic'"))
          fwrite(logfile_dt, file = logfile, append = T)
        }
      }
    }
...
las2dem = function(params_general, path_output = "", name_raster = "", kill, step, type_output = "tif", option = "dtm", spacing_pulses = as.numeric(NA)){
  ...
  if(!is.voidstring(params_general$path_lastools)){
    ...
    if(tolower(option) == "dtm"){
      rasterize_terrain(ctg, res = step, algorithm = tin()) # no kill option
    } else if(tolower(option) == "dsm"){
      rasterize_canopy(ctg, res = step,algorithm = dsmtin(max_edge = kill))
    } else {
      cat("Input option",option,"is not known. Please provide either DTM or DSM")
    }
  }
  ...
  if(nbnona > 0){
    ...
    for(i in 1:nrow(contours_pd)){
      ...
      if(file.size(file.path(dir_tile,paste0(contours_pd_current$id,"_sort.tif"))) > 0){
        ...
        dsm_spikefree = ifel(is.na(dsm_spikefree), dsm_spikefree_current, dsm_spikefree)
      }
  ...
  if(has.path_lastools == T){
    ...
    for(i in 1:length(clusters_data)){
      ...
      if(size_KB_input >= 10){
        ...
        crs_scan = terra::crs(classifications)
        ...
        if(length(classifications_unique) > 1){
          ...
          writeRaster(classifications, filename = file.path(path_output,paste0("classifications_supplied",addendum_name,".tif")), overwrite = T)
        }
        ...
        pulsedensity_smoothed = focal(pulsedensity_nona, w = 11, fun = "median", na.rm = T)
        ...
        for(scanangle_abs_current in c(20)){
          get.pulsedensity(params_general = params_general,path_output = file.path(params_general$path_data,paste0("pulsedensity",scanangle_abs_current)), step = resolution, scanangle_abs_max = scanangle_abs_current)
  
          files_pulsedensity_scanangle = list.files.nonzero(path = file.path(params_general$path_data,paste0("pulsedensity",scanangle_abs_current)), pattern = ".tif", full.names = TRUE)
          # using the virtual raster method from terra package
          ...
          unlink(x = file.path(params_general$path_data,paste0("pulsedensity",scanangle_abs_current)), recursive = T) # not needed for further processing
        }
  
        # pulse density, based on last returns
        get.pulsedensity(params_general = params_general,path_output = file.path(params_general$path_data,"pulsedensity_lastreturn"), step = resolution, keep_first = F)
        files_pulsedensity_lastreturn = list.files.nonzero(path = file.path(params_general$path_data,"pulsedensity_lastreturn"), pattern = ".tif", full.names = TRUE)
        # using the virtual raster method from terra package
        ...
        unlink(x = file.path(params_general$path_data,"pulsedensity_lastreturn"), recursive = T) # not needed for further processing
...
process.dataset = function(name_job, type_file, dir_dataset, dir_processed, tmpdir_processing, path_lastools, metadata = NULL, resolution = 1, n_cores = 4, size_tile = 500, size_buffer = 25, retile = T, cleanup = T, nbclusters_forced = NULL, force.utm = F, remove.buffer = F, remove.vlr = F, remove.evlr = F, factor_rescale = NULL, force.recompute = F, path_output_lazclean = "", path_output_laznorm = "", types_dsm = c("tin","lspikefree"), params_dsmadaptive = data.table(multi = 3.1, slope = 1.75, offset = 2.1), resolution_sumstatspc = NULL, add.timestamp = F, estimate.laserpenetration = F, type_os = "automatic", type_architecture = "64", by_file = F, perturbation_max = 0.1, timeout_lspikefree_max = 600, overwrite.crs = F, use.blast2dem = F, is.stdtime = NA, height_lim = 125, angle_lim = NULL, class_rm = c(), exclass_rm = c(), force.type_point = NULL, logfile = "", patterns_skip = c(), print.summary_job = F){
  ...
  # check LAStools
  has.path_lastools = check.path_lastools(path_lastools = path_lastools, logfile)
